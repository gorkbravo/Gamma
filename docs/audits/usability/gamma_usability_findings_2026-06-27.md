...
2. **GUA-20260627-3 (P1) — No fast in-app corroboration path for a single-day index dislocation. Status: Addressed in code 2026-07-08; live workflow retest recommended.** Blocked Thesis 1 step 4. To sanity-check Nikkei −4.15% I needed (a) the yen on the same screen (Sitrep FX strip was N/A), (b) a US-listed proxy's *latest daily* move (Equity Research Scope showed only lookback aggregates at audit time), and (c) a relevant headline (none). Code status: Research Overview nodes now carry `latest_daily_return` and `latest_daily_return_at`; Sitrep worldwide-index rows label/display latest-day change separately from lookback return and show proxy/as-of context; clicking Nikkei pivots directly to EWJ Scope Analysis so the Scope latest-day KPI can corroborate the dislocation. Remaining gap: retest with a live weekday provider session and separately verify GUA-20260627-4 for FX fallback/source labeling.
...
- Single-day vs lookback was a recurring blind spot: Equity Research, Sitrep, and historical audit-time Commodities each expressed change on a different time basis with no unifying "as-of / period" label the analyst could trust at a glance. Status update 2026-07-08: the Commodities time-basis defect is addressed; Equity Research Scope shows separate Lookback Return and Latest Day KPIs; Sitrep worldwide indices now use latest-day/as-of labels and a proxy pivot. Remaining gap: live workflow retest across a weekday market session.
...
- Status: **Addressed in code 2026-07-08; live workflow retest recommended.** Backend Research Overview metrics now preserve `total_return` separately from `latest_daily_return`/`latest_daily_return_at`. Sitrep worldwide-index rows display the latest daily close basis, daily absolute/% change, and proxy/as-of context, and mapped index rows pivot into Equity Research Scope Analysis using ETF proxies (`^N225` → `EWJ`, `^GSPC` → `SPY`, `^IXIC` → `QQQ`, etc.). Equity Research Scope already exposes Lookback Return and Latest Day KPIs from the scope-analysis response. Focused backend/frontend tests cover the new fields, labels, and Nikkei→EWJ handoff. Remaining gaps: weekday live-provider retest; FX-strip provider consistency remains tracked separately as GUA-20260627-4; event/headline search from an index tile is not implemented.
...
- GUA-20260627-3 is now addressed in code, so the highest-value remaining open fixes from this audit are GUA-20260627-5 (Sitrep perpetual REFRESHING / terminal unavailable state) and GUA-20260627-6 (agent/a11y-unfriendly controls). The Commodities `% CHG`, commodity daily-reference, and index-dislocation corroboration findings should still be rechecked against a live weekday session.
