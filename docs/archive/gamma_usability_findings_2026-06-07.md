...
- Completed (P2 related links, first pass): same-event prediction-market siblings are no longer presented as related on venue metadata alone. The `same_event` relationship now requires lexical overlap (similarity >= 0.25); below that the row becomes a `weak_venue_link` whose note states "Shares venue event metadata only... Likely unrelated." Weak links are excluded entirely whenever any semantically related row exists, and capped at three clearly-labeled rows when nothing better is available — so an AI-policy market no longer lists unrelated venue-event artifacts (the audited GTA VI case) alongside real matches. A full semantic/event taxonomy layer and user flagging of bad links remain future work.
- Fixed (found while verifying related links against live Polymarket data): the related-markets pipeline silently dropped every same-event sibling whose venue event payload carried no category metadata, because `_is_research_relevant_market` requires a canonical category and venue event endpoints often omit tags. An AI-model market family returned an empty related list even though all siblings were visible in the screener. Same-event siblings now inherit the opened market's research relevance; deliberately excluded sportsbook-style topics are still dropped via the headline exclusion keywords, and the new similarity gate demotes semantically unrelated artifacts. Verified live: `polymarket:996324` ("Will OpenAI have a #1 AI model by June 30?") went from zero related rows to its full labeled event family (conditional_consistency + same_event).
- Validation: backend 354 passed (plus new related-link and stale-handoff coverage), frontend 182 passed (2 new tests), `tsc --noEmit` clean, plus in-browser verification of the payoff and handoff fixes against the live IBKR session.

Progress update - 2026-06-12 (branch `claude/audit-completion-v2`):

- Completed (P1 provider/provenance badges, reference implementation): there is now a shared frontend provenance contract (`frontend/src/lib/provenance.ts`) and a reusable `ProvenanceBadge.svelte` component. The contract normalizes any payload-shaped provenance (`source_provider` / `freshness_label` / nested `freshness.status` / `retrieved_at` / `transformation_note` / `warnings`) into one badge datum with eleven states (live, delayed, cached, stale, historical, sample, synthetic, derived, model, unavailable, unknown); cached/sample/synthetic stay distinct instead of collapsing into "mocked". The badge renders `[STATE] provider · timestamp`, a `ƒ` marker when a transformation note exists, a `!n` warning count, and a full multi-line tooltip; tones are positive only for live, negative only for unavailable, warning for delayed/stale/cached/sample/synthetic/model.
- Adopted in three reference tabs instead of boiling the ocean: Equity Research (Overview "Coverage Notes" rail, Scope Analysis "Data Quality & Coverage" rail — which previously showed no provider/freshness at all — and the Strategy Lab import chart foot), Options (mode-bar source strip, Realized-vs-IV Boundary panel implied-source and history-freshness rows, Data & Source diagnostics panel), and Fundamentals (the four Source Trail rows: SEC company reference as `historical`, AI business summary as `model`, IBKR price context, Gamma derived analytics as `derived`). No new wrapper props were needed, so the EquityResearchView/StrategyLabView forwarding gotcha does not apply.
- Completed (P2 prediction-market taxonomy): added a `Tech/AI` research category. `ALLOWED_RESEARCH_CATEGORIES`, `EXACT_CATEGORY_ALIASES` (tech, technology, science and technology, ai, artificial intelligence), and `CATEGORY_KEYWORD_PATTERNS` now cover AI/tech entities (openai, anthropic, chatgpt, gpt, llm, agi, nvidia, semiconductors, robotics, spacex, ...). The Tech/AI keyword block is matched last so markets that already canonicalize to an existing category (e.g. "China AI chip export controls" -> Geopolitics) keep their labels; only previously-category-None markets are captured. The frontend screener category filter includes Tech/AI.
- Verified live in-browser on 2026-06-12: Equity Research Overview badge (`HIST yfinance` with the static-seed warnings in the tooltip), a live AAPL Scope Analysis run (`HIST yfinance !1`), Options degrading honestly without TWS (`N/A ibkr` + "Connect to IBKR" warning) and then, after connecting to live TWS on 7496 through the System toggle, a live AAPL max-depth surface showing `LIVE ibkr ƒ` in both the source strip and the diagnostics panel. Fundamentals NVDA loaded all four Source Trail badges live (`HIST sec`, `MODEL openai_responses`, `UNKNOWN ibkr` price context, `DERIVED gamma`). The Tech/AI screener filter returned live AGI/OpenAI-Anthropic-IPO/SpaceX markets that previously canonicalized to category None.
- Known gap found while verifying: Fundamentals headline metrics (e.g. IBKR price context) carry no `freshness_label`, so their badge state is honestly `unknown`; the backend fundamentals metric models should adopt freshness labels as part of wider badge adoption.
- Validation run: backend 355 passed (1 new Tech/AI canonicalization/precedence test), frontend 193 passed (11 new provenance-contract tests), `tsc --noEmit` clean.

Still left (provenance badges):

- Adopt the badge across the remaining tabs (Sitrep, Macro, Commodities, Crypto, Prediction Markets, Risk, Portfolio, Copilot context labels) and replace the per-view `tag-chip`/`badge-stack` freshness CSS in PredictionMarketsView/CryptoView with the shared component.
- Backfill `freshness_label` on backend payloads that lack it (Fundamentals metrics/company, Research scope results lack `retrieved_at`).
- Expand Strategy Lab per-leg source badges into the shared component (the P0 "still left" item).
- Prediction-market related links: user flagging/downranking of bad links remains future work (needs a small persistence layer and UI affordance).
