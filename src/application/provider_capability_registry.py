...
_ACTIVE_PROVIDER_CAPABILITIES: tuple[ProviderCapability, ...] = (
    ...
    _capability(
        provider_id="census",
        display_name="US Census",
        provider_class="official",
        status="optional",
        supported_domains=["macro", "trade_flows"],
        asset_classes=["trade", "goods_flows", "country_compare"],
        regions=["US"],
        data_types=["bilateral_trade", "partner_trade", "exports_ytd", "general_imports_ytd"],
        supports_delayed=True,
        supports_historical=True,
        freshness_levels=["official_release_lag", "historical", "cached"],
        historical_depth="Current Macro adapter uses Census International Trade NAICS country rows for US year-to-date goods trade partners.",
        credential_env_vars=["CENSUS_API_KEY"],
        configuration_notes=[
            "CENSUS_API_KEY enables the live US Trade Partners slice; Gamma falls back to curated rows if the adapter cannot fetch usable data.",
        ],
        limitations=[
            "Current adapter covers US country-level goods exports and general imports only.",
            "Services trade, revisions, commodity drilldowns, and country-group rows are outside this first live slice.",
        ],
        provenance_notes=[
            "Macro trade partner rows use source_provider='census' and origins like census.intltrade.naics:<YYYY-MM>.",
            "Gamma converts raw dollar values to USD billions and ranks by combined exports plus imports.",
        ],
        read_only_notes=["Official trade-flow research context only; no execution or account state."],
        source_provider_values=["census"],
        batch_fetching="monthly_exports_and_imports",
        background_refresh_safe=True,
    ),
    _capability(
...
_PLANNED_PROVIDER_CAPABILITIES: tuple[ProviderCapability, ...] = (
    ...
    _capability(
        ...
        credential_env_vars=["BLS_API_KEY"],
        configuration_notes=[
            "BLS public APIs can be used without a key for light usage, but BLS_API_KEY is reserved for higher-throughput official access.",
        ],
    ...
    _capability(
        ...
        freshness_levels=["official_release_lag", "historical"],
        historical_depth="Candidate dedicated macro-series adapter; current use is limited to official event schedule parsing.",
        credential_env_vars=["BEA_API_KEY"],
        configuration_notes=[
            "BEA_API_KEY is reserved for GDP, national accounts, income, and US trade adapter work.",
        ],
    ...
    _capability(
        ...
        credential_env_vars=["NEWS_PROVIDER", "RSS_NEWS_TIMEOUT_SECONDS"],
        configuration_notes=[
            "Set NEWS_PROVIDER=rss to use the curated source-owned RSS feed adapter for normalized news/event records.",
            "Default runtime news coverage remains sample/offline unless RSS is explicitly configured.",
            "RSS_NEWS_TIMEOUT_SECONDS controls the per-feed HTTP timeout.",
        ],
        limitations=[
            ...
            "Feed fetches fail per source and return warnings rather than making the entire news service unavailable.",
        ],
        provenance_notes=[
            "RSS records preserve feed URL/feed origin, source domain, provider item id when available, publication timestamp, retrieval timestamp, and source URL.",
    ...
    _capability(
        ...
        regions=["EU"],
        data_types=["policy_calendar", "rates_series", "eu_macro_series", "exchange_rates"],
        supports_delayed=True,
        supports_historical=True,
        freshness_levels=["official_release_lag", "historical"],
        historical_depth="Candidate dedicated EU rates and macro provider; current use is limited to ECB policy-calendar parsing.",
        credential_env_vars=["ECB_API_KEY"],
        configuration_notes=[
            "Most ECB statistical endpoints are public; ECB_API_KEY is a reserved placeholder for future throttled or proxied access.",
        ],
    ...
    _capability(
        ...
        status="planned",
        supported_domains=["macro"],
        asset_classes=["economic_series", "inflation", "growth", "labor", "trade"],
        regions=["EU", "Europe"],
        data_types=["eu_macro_series", "country_series", "regional_statistics", "trade_balance", "partner_trade"],
        supports_delayed=True,
        supports_historical=True,
        freshness_levels=["official_release_lag", "historical"],
        historical_depth="Candidate EU macro provider; depth depends on dataset.",
        credential_env_vars=["EUROSTAT_API_KEY"],
        configuration_notes=[
            "Eurostat is the primary planned source for EU country-compare rows and intra/extra-EU trade context.",
        ],
    ...
    _capability(
        ...
        status="planned",
        supported_domains=["macro"],
        asset_classes=["economic_series", "fx", "external_accounts", "country_compare"],
        regions=["Global", "US", "EU"],
        data_types=["country_macro_series", "balance_of_payments", "current_account", "fx_reserves", "cross_country_indicators"],
        supports_delayed=True,
        supports_historical=True,
        freshness_levels=["official_release_lag", "historical"],
        historical_depth="Candidate global macro comparator where US/EU official sources need harmonized international context.",
        credential_env_vars=["IMF_API_KEY"],
        configuration_notes=[
            "IMF_API_KEY is a reserved placeholder; many IMF data APIs are public but adapter usage and throttling need validation.",
        ],
        limitations=[
            "No Gamma adapter is implemented yet.",
            "Indicator frequency, country-code mapping, and vintage/revision semantics need normalized schemas before live use.",
        ],
        provenance_notes=[
            "Future IMF records should preserve dataset code, indicator code, country code, frequency, unit, and Gamma harmonization notes."
        ],
        read_only_notes=["Official macro data only."],
        source_provider_values=["imf"],
        batch_fetching="planned_dataset_queries",
        background_refresh_safe=True,
    ),
    _capability(
        ...
        status="planned",
        supported_domains=["macro"],
        asset_classes=["economic_series", "labor", "inflation", "country_compare"],
        regions=["Global", "US", "EU"],
        data_types=["harmonized_indicators", "country_macro_series", "leading_indicators", "labor_statistics"],
        supports_delayed=True,
        supports_historical=True,
        freshness_levels=["official_release_lag", "historical"],
        historical_depth="Candidate harmonized country-compare provider for advanced economy metrics.",
        credential_env_vars=["OECD_API_KEY"],
        configuration_notes=[
            "OECD_API_KEY is a reserved placeholder for future keyed, proxied, or rate-limited adapter access.",
        ],
        limitations=[
            "No Gamma adapter is implemented yet.",
            "Dataset selection and code mapping need to be explicit so comparisons do not mix incompatible units or frequencies.",
        ],
        provenance_notes=[
            "Future OECD records should preserve dataset, dimension keys, unit, frequency, and any Gamma country/region harmonization."
        ],
        read_only_notes=["Official macro data only."],
        source_provider_values=["oecd"],
        batch_fetching="planned_dataset_queries",
        background_refresh_safe=True,
    ),
    _capability(
        ...
        asset_classes=["trade", "goods_flows", "country_compare"],
        regions=["Global", "US", "EU"],
        data_types=["bilateral_trade", "partner_trade", "goods_categories", "trade_balance"],
        supports_delayed=True,
        supports_historical=True,
        freshness_levels=["official_release_lag", "historical"],
        historical_depth="Candidate source for country trade-partner tables and goods-flow composition.",
        requires_api_key=True,
        credential_env_vars=["UN_COMTRADE_API_KEY"],
        limitations=[
            "No Gamma adapter is implemented yet.",
            "Reported vs partner mirror flows, commodity-code aggregation, and revision policy need explicit labeling.",
        ],
        provenance_notes=[
            "Future UN Comtrade records should preserve reporter, partner, flow, commodity code, period, unit, and mirror-flow treatment."
        ],
        read_only_notes=["Official trade-flow research context only."],
        source_provider_values=["un_comtrade"],
        batch_fetching="planned_dataset_queries",
        background_refresh_safe=True,
    ),
    _capability(
        ...
        status="planned",
        supported_domains=["macro", "trade_flows"],
        asset_classes=["trade", "global_trade", "policy_context"],
        regions=["Global", "US", "EU"],
        data_types=["trade_indicators", "tariff_context", "trade_policy_context", "country_trade_series"],
        supports_delayed=True,
        supports_historical=True,
        freshness_levels=["official_release_lag", "historical"],
        historical_depth="Candidate global trade indicator and policy-context source.",
        credential_env_vars=["WTO_API_KEY"],
        configuration_notes=[
            "WTO_API_KEY is a reserved placeholder; public endpoint behavior and key requirements need validation during adapter work.",
        ],
        limitations=[
            "No Gamma adapter is implemented yet.",
            "Policy-context fields should stay descriptive and must not be converted into unexplained trade scores.",
        ],
        provenance_notes=[
            "Future WTO records should preserve indicator, reporter, partner or region scope, period, unit, and policy-context source links."
        ],
        read_only_notes=["Official trade-policy and trade-indicator research context only."],
        source_provider_values=["wto"],
        batch_fetching="planned_dataset_queries",
        background_refresh_safe=True,
    ),
    _capability(
