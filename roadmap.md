...
This table tracks the visible app tabs as of 2026-08-25. Percentages are pragmatic implementation snapshots, not release promises.
...
| Research | `Copilot` | **~85% toward clarified end state; Checkpoint 8 in progress** | Checkpoints 8A-8C now cover unloaded single-name Fundamentals, public-company identity acquisition, and user-specified temporary portfolio/Risk workflows. `LMT` or a validated natural company name can run without tab preload; ambiguous share classes stop with candidates; and an exact portfolio such as `60% AAPL / 40% TLT` can carry `+100 bps` and `-10% equity` shocks into one restart-safe, visibly temporary Risk analysis without changing the live portfolio. Next, generalize the same contract to Options, Strategy Lab, and remaining tab-owned analyses before approval/resume and release hardening. |
...
_Status: In progress (~85% toward the clarified end state; checkpoints 1-7 and Checkpoints 8A-8C are verified)_
...
| 8. Entity acquisition and working-analysis state | **89%** | Make core tools entity-addressable; add session-scoped ephemeral portfolios, DCFs, risk scenarios, option sets, assumptions, and intermediate outputs; materialize useful results through typed owning-tab state/handoffs. Public-company identity, unloaded reverse valuation, and hypothetical-portfolio/Risk state are complete through Checkpoint 8C; Options, Strategy Lab, and remaining owning surfaces are still open. | An unloaded-`LMT` or natural-name fair-value request and a user-specified hypothetical portfolio/risk request now complete without manual tab preloading. Ambiguous issuer/share-class names stop for an explicit ticker. The remaining exit work is to apply the same visibly temporary, restart/expiry-safe, non-trading contract to the other representative tab-owned analyses. |
