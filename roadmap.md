...
| Research | `Strategy Lab` | ~90% | Deepen Gamma object composition and saved-run workflows; fix composer state-loss traps (legs reset on tab switch, silent compose no-op before validation). The isolated Research Script Workspace is complete: bounded Gamma exports, immutable source/input runs, typed retained outputs, comparison/export/lifecycle/recovery UX, OpenAI Code Interpreter with network disabled, and mock/live/disabled acceptance are verified. |
