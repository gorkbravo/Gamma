...
| Research | `Copilot` | **~72% toward clarified end state** | The verified chat/session/context/tool/permission foundation remains intact, but the July 26 scope clarification requires a true closed-loop Research Operator: unloaded-entity acquisition, schema-faithful parameter translation, session-scoped working analyses, observation-driven replanning, final synthesis from tool outputs, app materialization, and generalized approval/resume. Release hardening follows those capabilities. |
...
_Status: In progress (~72% toward the clarified end state; checkpoints 1-6 remain verified foundation)_
...
- `Dedicated workspace`: approximately 72% toward the clarified end state. The typed chat/session/artifact, context/model-policy, exact shelf promotion, retention/routing observability, diagnostics, and narrow Operator confirmation foundation are verified. The main remaining work is the closed-loop Operator, entity-addressable acquisition, generalized working state, app materialization, approval/resume, and end-state acceptance gate.
...
The earlier 97% was a legitimate verification mark for the narrower checkpoints 1-6 foundation, but it is not the completion percentage for the clarified Operator product. The current approximately 72% baseline credits that delivered foundation while recognizing that adaptive app operation is a material part of the target. Percentage should advance only when a checkpoint's exit criteria are implemented and verified; it is a product-outcome gate, not a code-volume estimate.

| Checkpoint | Target | Required delivery | Exit criteria |
| --- | ---: | --- | --- |
| Current clarified baseline | **~72% (2026-07-26)** | Preserve checkpoints 1-6: shared streaming/run/transcript/session/artifact contracts, context and evidence, action registry and narrow DCF approval, typed read-only tools, exact shelf continuity, model/storage policy, observability, and diagnostics. | Existing verification remains valid. Documentation and evals explicitly identify that the default Operator is mostly deterministic, argument/entity handling is shallow, working state and same-run resume are narrow, and final synthesis is not consistently coupled to tool results. |
| 7. Closed-loop Operator core | **82%** | Implement a bounded model-tool-model loop; strict model-produced tool arguments with deterministic validation; observation-driven next-step selection; explicit budgets/stopping states; final synthesis from actual outputs. | Traces prove that the Operator can preserve user parameters, inspect a result, adapt or stop appropriately, and answer the requested goal. Generic step-count summaries and unrelated fallback arguments fail the gate. The registry remains authoritative in both custom Responses and Agents SDK variants. |
