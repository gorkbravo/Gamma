...
| Research | `Copilot` | 80% | Add in-tab memo/report editing and export, finish session lifecycle and retention controls, productionize inline Operator confirmations and Agents SDK live events, expand Sealanes/news and priority domain drilldowns, add shelf-to-workspace continuity, and pass the Copilot release gate. Checkpoint 2 now provides one typed transcript/evidence renderer across shelf and workspace, pre-persistence claim/source validation, and context-preserving claim navigation. |
...
_Status: In progress (80%; checkpoints 1-2 verified)_
...
| --- | ---: | --- | --- |
| Current verified baseline | 80% | Checkpoints 1-2: server-owned typed Agent/Operator runs with replay and cancellation; one transcript/evidence contract across shelf and workspace; pre-persistence claim/source resolution; context-preserving source navigation; existing session, artifact, and Operator foundations. | Checkpoint 2 gate passed with 89 focused backend/eval tests, 267 frontend tests, typecheck, production build, and desktop check; live desktop inspection covered ready/error/cancelled transcripts plus a claim-source handoff to Macro Snapshot preserving the 3M timeframe, with zero console errors. |
...
1. Checkpoints 1 and 2 are complete. Their shared run-event and transcript/evidence contracts are the required foundation for all later work.
