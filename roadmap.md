...
_Status: In progress (91%; checkpoints 1-4 verified)_
...
- `Dedicated workspace`: ~91% complete. In addition to the typed chat/session/artifact contract, Operator turns now render exact pending mutations and before/after diffs inline, explain the pre-change snapshot policy, and expose apply/reject controls that reconcile into live and replayed turns. Remaining workspace work belongs to Checkpoint 5 and later.
...
| --- | ---: | --- | --- |
| Current verified baseline | **91%** | Checkpoints 1-4: the 86% run/transcript/session/artifact baseline plus one authoritative permission registry, durable context-bound single-use confirmation tokens, restart-safe DCF proposal/apply/reject/expiry, inline diff/snapshot/apply/reject UX, Agents SDK live progress, and safe-turn cancellation. | Checkpoint 4 gate passed with 104 backend/eval tests and 309 frontend tests plus typecheck, production build, and desktop check. Adverse cases cover wrong token/session/context/hash, expiry, replay, restart, forbidden actions, partial tool failure, budget enforcement, provider failure, custom safe-step cancellation, and Agents SDK `after_turn` cancellation. |
...
1. Checkpoints 1 through 4 are complete. Their shared run-event, transcript/evidence, session/replay, artifact, permission, and confirmed-mutation contracts are the required foundation for all later work.
