...
| Research | `Copilot` | **86% verified** | Checkpoint 3 now provides typed session rename/archive/restore/delete, schema migration and recovery, full replayable turn state, and an in-tab memo/report workflow with provenance-preserving Markdown export. Next, resolve the small post-checkpoint session/composer/storage-warning regressions in [`docs/copilot_v2_checkpoint3_prompt.md`](./docs/copilot_v2_checkpoint3_prompt.md), then productionize inline Operator confirmations and live progress in Checkpoint 4. Later blockers remain context/tool gaps, shelf-to-workspace continuity, model/retention diagnostics, and the release gate. |
...
_Status: In progress (86%; checkpoints 1-3 verified)_
...
_Recent progress: The dedicated Copilot tab was rebuilt in June 2026 as a standard chat interface with session search/archive/new-chat, a pinned composer, Agent/Operator controls, and multi-select context scope. On July 14, the Agent path gained provider-native Responses streaming behind one Gamma NDJSON run-event contract. On July 15, finalized Agent results gained a discriminated transcript-block model and the dedicated tab reached parity with the shelf for all current ResearchCard fields and evidence. On July 17, checkpoint 1 replaced raw `urllib` transport with the supported OpenAI SDK, made runs server-owned with bounded cursor replay across disconnects, added completed function-argument and provider-error events, moved shelf Agent and custom-loop Operator work onto the shared run/reducer contract, added safe-boundary Operator cancellation, and verified idempotent single-terminal persistence across cancellation, timeout, refusal, incomplete output, provider failure, duplicate/stale/post-terminal events, and reconnect. On July 24, checkpoint 2 unified cards, plans, Operator steps/results, reports, confirmations, diffs, artifacts, and typed non-success states behind one transcript renderer; normalized claim evidence before return, persistence, and legacy reads; and added claim-level `CrossTabHandoffEnvelope` navigation that preserves supported entity, mode, timeframe, and lens context. On July 25, checkpoint 3 completed the typed session lifecycle, schema-v3 migration and non-destructive recovery policy, complete turn/restart replay, and the dedicated in-tab memo/report workflow with source-turn selection, templates, autosave, preview, duplicate/delete, and provenance-preserving Markdown export. Automated evidence comprised 92 focused backend tests, 3 Agents SDK/operator eval tests, and 274 frontend tests plus typecheck, production build, and desktop check. User-supplied desktop and narrow-width inspection confirmed lifecycle actions and cancellation, artifact editing, duplication/deletion, source-turn controls, responsive inspector layout, and honest quota-exhausted and unsupported-tool states. The provider smoke did not produce a successful live answer because the configured OpenAI account had no remaining quota; this is recorded as an external provider limitation rather than fabricated success. On July 25 a focused post-checkpoint pass closed the three recorded regressions without advancing the checkpoint: `New chat` now creates one authoritative empty session through `POST /copilot/sessions` before selecting it, so it no longer reconciles into an existing conversation; selected, inactive, running, and archived sessions are presented as distinct states and switching away from a streaming run neither cancels it nor moves its transcript; the composer clears once a submission is accepted (including quota, provider, refusal, incomplete, cancellation, and zero-tool outcomes) while pre-acceptance rejections preserve the draft and Retry uses the persisted turn; and the storage-recovery warning moved from an absolutely positioned banner into an in-flow, dismissible status strip with safe record details that stays rediscoverable from a `Storage` control. The regression list is recorded in [`docs/copilot_v2_checkpoint3_prompt.md`](./docs/copilot_v2_checkpoint3_prompt.md). The detailed engineering spec remains [`docs/copilot_v2_tab_plan.md`](./docs/copilot_v2_tab_plan.md); the milestone sequence and percentage gates below are the authoritative roadmap path to 100%._
...
- `Dedicated workspace`: ~86% complete. The standard chat interface now has typed session lifecycle controls, a pinned composer, Agent/Operator controls, multi-select context scope, real provisional provider deltas, Stop/Retry, typed non-success states, full finalized ResearchCard/evidence parity, and a responsive artifact inspector. The July 25 follow-up added an authoritative `New chat` create/select contract, distinct selected/inactive/running/archived presentation, acceptance-driven composer clearing with draft preservation before acceptance, and a non-obstructive storage-recovery status strip. Remaining workspace work belongs to Checkpoint 4 and later.
...
- `Diagnostics, routing, and release evidence`: ~48% complete. Provider configuration and usage foundations exist, Agent/Operator test/eval paths are present, and restart/recovery coverage now exercises mixed-version and corrupted storage. Remaining work: versioned model policy, resolved routing/usage/latency visibility, retention controls, structured diagnostic ids, accessibility completion, and representative successful live smoke when provider quota is intentionally available.
...
| --- | ---: | --- | --- |
| Current verified baseline | **86%** | Checkpoints 1-3: server-owned typed Agent/Operator runs with replay and cancellation; one transcript/evidence contract across shelf and workspace; pre-persistence claim/source resolution; context-preserving source navigation; complete typed session lifecycle, migration/recovery, restart replay, and in-tab memo/report artifacts. | Checkpoint 3 gate passed with 95 focused backend/eval tests, 274 frontend tests, typecheck, production build, and desktop check. Temporary-store restart/export/mixed-corruption fixtures passed. User-supplied desktop and narrow-width inspection confirmed lifecycle and artifact workflows; the only observed provider failure was the explicit OpenAI quota-exhausted state, and SITREP Operator truthfully reported that no registered automatic tools were available. |
...
1. Checkpoints 1 through 3 are complete. Their shared run-event, transcript/evidence, session/replay, and artifact contracts are the required foundation for all later work.
...
3. Checkpoint 4 may deepen read-only Operator coverage, but no new durable mutation family should ship before inline confirmation and the existing persistence/replay authority are reliable.
