...
| Research | `Copilot` | **97% verified** | Checkpoints 1-6 now provide the shared run/transcript/session/artifact foundation, productionized Operator authority, versioned context and model-policy contracts, exact idempotent shelf promotion, local/provider retention policy, replayable usage/routing observability, and safe provider diagnostics. The retained eval did not justify changing the GPT-5.5/custom-Operator defaults. Next: Checkpoint 7 accessibility, first-run, responsive/live-provider release evidence, and the final 100% gate. |
...
_Status: In progress (97%; checkpoints 1-6 verified)_
...
- `Dedicated workspace`: ~97% complete. The typed chat/session/artifact and Operator confirmation foundation now includes replayable context/model-policy contracts, selected/omitted-domain explanations, deterministic compaction disclosures, exact shelf promotion, retention/routing observability, safe diagnostics, and a run inspector. Remaining workspace work belongs to the Checkpoint 7 release gate.
...
- `Research Operator`: checkpoint 4 complete. The authoritative registry rejects unsupported automatic work and prohibited execution-capable families; bounded read-only analyses run automatically; DCF is the single narrow durable mutation family and requires an exact active token bound to session, context, tool, and proposal; proposals, expiry, rejection, apply, rollback snapshot, and replay state are persisted. Custom and Agents SDK paths emit the shared live trace contract and cancel at safe boundaries. The custom loop remains default pending a measured live SDK advantage.
- `Diagnostics, routing, and release evidence`: checkpoint 6 foundations complete. Versioned model/storage policy, resolved routing/usage/latency/call/cancellation replay, explicit unavailable metrics, safe diagnostic ids, and Settings/run-inspector guidance are implemented. Remaining work: Checkpoint 7 accessibility/focus/reduced-motion and narrow-layout completion, first-run polish, and representative successful live Agent/Operator smoke when access and spend are intentionally authorized.
...
| --- | ---: | --- | --- |
| Current verified baseline | **97%** | Checkpoints 1-6: the shared run/transcript/session/artifact and Operator-authority foundation; versioned context and model/storage policies; exact idempotent shelf continuity; local stateless continuation; replayable routing/usage/cancellation observability; and safe Settings/run-inspector diagnostics. | Checkpoint 6 passed 125 focused Copilot/Agents/eval tests, 50 affected runtime/provider tests, the full 468-test backend suite, 320 frontend tests, typecheck, production build, desktop check, and 31/31 deterministic scored eval outcomes across 11 retained cases and five routing variants. No live-provider variant was authorized; missing provider metrics remain unavailable and the GPT-5.5/custom-Operator defaults remain unchanged. |
...
| 2. Complete transcript and evidence contract | **80% verified 2026-07-24** | Extended discriminated transcript blocks to plans, Operator steps/results, reports, confirmations, diffs, artifacts, and typed non-success states; resolved claim evidence to known source ids; added source/context navigation; visibly separated source-backed claims, inference, assumptions, missing data, and warnings. | Result evidence is normalized before return and persistence and revalidated on legacy reads. Shelf and workspace share the canonical renderer. Card, plan, Operator, report, confirmation, mutation, artifact, error, and navigation tests pass; a live source click opened Macro Snapshot and retained the 3M timeframe. |
| 3. Finish sessions and in-tab artifacts | **86% verified 2026-07-25** | Added session rename, archive/restore, delete, schema-v3 migration/recovery, and faithful persistence of role/depth/scope/model/run/usage/artifact state. Added memo/report source-turn selection, concise/full templates, edit/autosave, canonical preview, duplicate/delete, explicit overwrite, and Markdown export inside the workspace. | Restart fixtures faithfully reopen transcript, context snapshots, runs, plans, traces, confirmations, memos, and reports. Export fixtures preserve claim labels, inline evidence refs, source metadata, warnings, provider/model metadata, and source-turn links. Desktop and narrow-width user inspection confirmed the lifecycle and artifact surfaces; follow-up polish issues are tracked separately and do not invalidate the persisted Checkpoint 3 contracts. |
| 4. Productionize Research Operator | **91% verified 2026-07-25** | Stream Operator progress live; cancel only between safe steps; render inline confirmation checkpoints with before/after diff and rollback/snapshot context; retain one authoritative action registry and permission path; expand failure, stale-context, partial-tool, repeated-confirmation, cancellation, and resume evals. | Bounded read-only analyses run automatically, while every durable/non-trivial local research-state mutation stops on the exact active confirmation token. No trade, order, account, rebalance, wallet, or arbitrary-code capability exists. The retained custom default is documented: Agents SDK now has progress/cancellation/permission parity, but no new measured live quality or latency advantage justifies switching yet. |
| 5. Close context and tool-coverage gaps | **94% verified 2026-07-25** | Added Sealanes context/read-only drilldowns without invented risk labels; made news item-level context first class; added priority IV, Commodities, and Equity Research drilldowns; implemented per-scope/aggregate budgets, deterministic compaction, stale-context invalidation, and navigation mappings/fingerprints for every selectable scope. | NVDA, CPI/Fed, oil-disruption, and portfolio-rate-shock requests select the expected domains and explain omissions. Provenance, freshness, warnings, omissions, navigation, and context metadata survive compaction, degradation, persistence, and replay. Every new tool passes the server registry; unsafe citations and execution/account/wallet capabilities remain absent. Gate evidence: 209 focused backend tests, 461 full backend tests, 315 frontend tests, typecheck/build/desktop check, and 22/22 deterministic eval outcomes. |
| 6. Finish continuity, model policy, retention, and diagnostics | **97% verified 2026-07-25** | Added exact reference-based `Open in Copilot`; `copilot.model-policy.v1` Auto/Quick/Standard/Deep resolution; `copilot.provider-storage.v1`; schema-v4 routing/usage/latency/cache/call/cancellation replay; tested `store: false` local continuation; and safe provider/model/storage diagnostics in Settings and the run inspector. | Promotion preserves the original session, turns, snapshots, context contracts/fingerprints, domain decisions, evidence, warnings, freshness, and scope boundary without duplication and is idempotent across restart. Unsupported combinations degrade explicitly. Missing provider metrics remain null. The 31/31 deterministic scored eval retained GPT-5.5 and the custom Operator because no authorized live comparison demonstrated an advantage. No credential, raw provider payload, authorization value, unnecessary raw prompt, or private reasoning is persisted or displayed. |
...
1. Checkpoints 1 through 5 are complete. Their shared run-event, transcript/evidence, session/replay, artifact, permission, confirmed-mutation, context-budget, domain-routing, and source-navigation contracts are the required foundation for all later work.
