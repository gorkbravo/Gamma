...
| Research | `Copilot` | **~82% toward clarified end state; Checkpoint 8 in progress** | Checkpoints 8A-8B now cover the unloaded single-name Fundamentals slice plus public-company identity acquisition: `LMT` can run without tab preload, natural names such as Apple are model-proposed and SEC-validated into canonical tickers, ambiguous share classes stop with candidates, and reverse valuation creates versioned session-ephemeral state that materializes visibly but non-durably in Fundamentals. Next, generalize the same contract to hypothetical portfolios and Risk, then the remaining tab-owned analyses, before approval/resume and release hardening. |
...
| 8. Entity acquisition and working-analysis state | **89%** | Make core tools entity-addressable; add session-scoped ephemeral portfolios, DCFs, risk scenarios, option sets, assumptions, and intermediate outputs; materialize useful results through typed owning-tab state/handoffs. Public-company natural names now pass through model proposal plus authoritative SEC validation, with typed ambiguity stops. | An unloaded-`LMT` or natural-name fair-value request and a user-specified hypothetical portfolio/risk request complete without manual tab preloading. Ambiguous issuer/share-class names stop for an explicit ticker. Temporary state is visibly distinct from durable saved state, survives or expires by policy, and never rebalances or trades. |
