...
- `Cross-tab handoffs`: ~68% complete. Tables and triage rows route into Research, Macro, Commodities, and Prediction Markets; handoff sessions expire into an earlier-sessions queue; and starred triage rows persist locally as follow-ups whose saved handoffs reopen the originating tab/mode. Remaining work is richer lens/entity preservation.
- `Saved triage / follow-ups`: ~70% complete. What Changed and Events & Markets rows can be starred into a locally persisted Follow-Ups panel (capped, schema-validated, dismissable, handoff-preserving). Remaining work is follow-up notes, resolved/done states, and eventual backend persistence alongside the SITREP contract.
